...
with col1:
    ...
    if st.button("View journey"):
        # Filter only Customer transactions for journey table
        customer_journey = [
            {
        ...
        if customer_journey:
            # Show journey table with mainly status
            st.write("### Product Journey (Customer Status)")
            df_journey = pd.DataFrame(customer_journey)
            st.dataframe(df_journey)

            # Show Customer Summary with full details
            customer_records = [
                {
            ...
            df_summary = pd.DataFrame(customer_records)

...
# ==============================
class Transaction:
    def __init__(self, product_id, product_name, role, actor_name, location, status, notes, payment_method=None, payment_amount=0):
        ...
        self.payment_method = payment_method  # "UPI", "Cash on Delivery", "Card", "Net Banking"
...
if st.button("View journey"):
    ...
    if journey:
        ...
        last_tx = journey[-1]  # Show the latest transaction info
        ...
        st.write(f"*Payment Amount:* ₹{last_tx['Payment Amount']}")
    else:
...
with st.sidebar:
    ...
    actor_name = st.text_input("Actor name (Customer/Supplier)")
    ...
    payment_method = st.selectbox("Payment Method", ["None", "UPI", "Cash on Delivery", "Card", "Net Banking"])
    payment_amount = st.number_input("Payment Amount", min_value=0, value=0)

    if st.button("✅ Submit transaction"):
        if product_id and product_name and actor_name:
            pm = None if payment_method == "None" else payment_method
            tx = Transaction(product_id, product_name, role, actor_name, location, status, notes, pm, payment_amount)
            blockchain.add_block(tx)
            st.success("Transaction added to blockchain!")
        else:
            st.error("Please enter Product ID, Product Name, and Actor name.")
